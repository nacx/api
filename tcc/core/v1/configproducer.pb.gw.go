...
// RegisterConfigProducerHandlerFromEndpoint is same as RegisterConfigProducerHandler but
// automatically dials to "endpoint" and closes the connection when "ctx" gets done.
func RegisterConfigProducerHandlerFromEndpoint(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) (err error) {
	...
	return RegisterConfigProducerHandler(ctx, mux, conn)
}

// RegisterConfigProducerHandler registers the http handlers for service ConfigProducer to "mux".
// The handlers forward requests to the grpc endpoint over "conn".
func RegisterConfigProducerHandler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn) error {
	return RegisterConfigProducerHandlerClient(ctx, mux, NewConfigProducerClient(conn))
}

// RegisterConfigProducerHandlerClient registers the http handlers for service ConfigProducer
// to "mux". The handlers forward requests to the grpc endpoint over the given implementation of "ConfigProducerClient".
// Note: the gRPC framework executes interceptors within the gRPC handler. If the passed in "ConfigProducerClient"
// doesn't go through the normal gRPC flow (creating a gRPC client etc.) then it will be up to the passed in
// "ConfigProducerClient" to call the correct interceptors.
func RegisterConfigProducerHandlerClient(ctx context.Context, mux *runtime.ServeMux, client ConfigProducerClient) error {

	mux.Handle("GET", pattern_ConfigProducer_Download_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		...
		resp, md, err := request_ConfigProducer_Download_0(rctx, inboundMarshaler, client, req, pathParams)
		...
		forward_ConfigProducer_Download_0(ctx, mux, outboundMarshaler, w, req, resp, mux.GetForwardResponseOptions()...)
...
var (
	forward_ConfigProducer_Download_0 = runtime.ForwardResponseMessage
)
